...
// Fetch all Barangay appointments
...
if (isset($_POST['approve'])) {
    ...
    if ($stmt->execute()) {
        // Get user_id from appointment to insert notification
        ...
        $message = "Your Barangay appointment has been approved.";
        ...
        $_SESSION['notification'] = "Your Barangay appointment has been approved.";
...
if (isset($_POST['decline'])) {
    $id = $_POST['id'];
    $update_sql = "UPDATE appointments SET status = 'declined' WHERE id = ?";
    $stmt = $conn->prepare($update_sql);
    $stmt->bind_param('i', $id);
    ...
    if ($stmt->execute()) {
        ...
        // Insert notification into the notifications table
        $message = "Your Barangay appointment has been declined.";
        $insert_notif_sql = "INSERT INTO notifications (user_id, message) VALUES (?, ?)";
        $insert_notif_stmt = $conn->prepare($insert_notif_sql);
        $insert_notif_stmt->bind_param('is', $user['user_id'], $message);
        $insert_notif_stmt->execute();

        $_SESSION['notification'] = "Your Barangay appointment has been declined.";
    ...
    // Redirect to prevent resubmission
    header("Location: applicationappointment.php");
    exit;
}
?>
...
<meta charset="utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
<title>Barangay Appointments</title>
...
<div class="container mt-5">
  <h1 class="mb-4">Barangay Appointments</h1>
      ...
      <?php if ($result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td><?= htmlspecialchars($row['appointment_with']) ?></td>
